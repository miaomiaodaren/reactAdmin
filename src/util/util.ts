...
// 用来计算缓存方法  取自 vue 源码
// function computed(str) {// 假设中间的计算非常耗时 console.log('2000s have passed') return 'a result' } 
// var cachedComputed = cached(computed)  cachedComputed('ss')  // 打印2000s have passed cachedComputed('ss')  // 不再打印
...
// 使a = 10, b = 11, c = 12, 以此类推 (目前只支持小写)
export const getCode = (chars: string) => {
    var r = (chars.length - 1) * 26;
    for (var i = 0; i < chars.length; i++) {
        r += chars.charCodeAt(i) - 87;
    }
    if (chars.length > 1) {
        r -= chars.length - 1;
    }
...
// https://www.jianshu.com/p/54fc0fce46cc
...
// 实现二个数组各位数之间相加
export const arrSum = (arr1: Array<number>, arr2: Array<number>) => {
    return arr1.map((item: number, index: number) => {
        return item + arr2[index];
    });
};
// [[1,2,3,4], [2,3,4,5]] === [3,4,7,9]
export const moreArrSum = (arr: Array<Array<number>>) => {
    return arr.reduce((item: number[], arr: number[], index: number) => {
        if(index === 0) {
            return item;
        } else {
...
// js 数组换位置多个方法  [1,2,3,4,5]
export const changeArrIndex = (arr: any[], first: number, end: number) => {
    // 如果已知二者交换位置
    return arr.splice(first - 1, 1, ...arr.splice(end - 1, 1, arr[first - 1]));
    // 可以使用中间件
